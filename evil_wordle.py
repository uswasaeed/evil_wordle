...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for i, letter in enumerate(guessed_word):
            ...
            elif (feedback_colors[i] == WRONG_SPOT_COLOR and
                  self.colors[letter] not in (CORRECT_COLOR, WRONG_SPOT_COLOR)):
    ...
    def __str__(self):
        ...
        return "\n".join(
            " " * spaces[i] + " ".join(
                color_word(self.colors.get(char, NO_COLOR), char) for char in row)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for letter in secret_word:
        secret_count[letter] = secret_count.get(letter, 0) + 1
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            feedback[i] = CORRECT_COLOR
            secret_count[guessed_word[i]] -= 1
    for i in range(NUM_LETTERS):
        if (feedback[i] == NOT_IN_WORD_COLOR and guessed_word[i] in secret_count
            and secret_count[guessed_word[i]] > 0):
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    """
    try:
        input_file = os.readlink(f"/proc/{os.getpid()}/fd/0")
        secret_word = os.path.splitext(os.path.basename(input_file))[0]
        remaining_secret_words = [secret_word]
    ...
    for word in remaining_secret_words:
        ...
        if feedback not in family_map:
            family_map[feedback] = []
        family_map[feedback].append(word)

    word_families = [WordFamily(feedback, words) for feedback, words in family_map.items()]
